...
const TV_SEARCH_SOURCES_RAW = [
  ...
  /* douapi 已摘除（2026-09-23，用户实测后决定不用）。
     https://api.douapi.cc/api.php/provide/vod/
     它是那次体检里唯一「独有贡献 100/100」的源，看着最该加，
     但逐条核对发现 43% 是成人片 —— 独有度满分恰恰是因为正规源不收这些。
     教训：只按「独有度」筛源会直接把黄站选进来，必须同时查内容成分。 */
]
...
/* 给用户看的源名，和 TV_SOURCES_RAW 一一对应，顺序不能动。
   ⚠ 注意映射关系是 TV_ALL_RAW = TV_SEARCH_SOURCES_RAW.concat(TV_LIST_ONLY_RAW)，
     ...
     名字就要插在**同一个位置**，不能图省事追加到末尾 ——
     追加会让「无损云」拿到「亚洲」的名字，后面全体错位一格。
     当前对应关系：0-6 原有七个 / 7-9 亚洲如意魔都 / 10-12 仅列表三个。 */
const TV_NAMES_RAW = [
...
/* 采集站几乎都挂着一个成人栏目；本站不展示，直接在网关里摘干净（前端 js/tv.js 还有一层）
   ...
   ============================================================
   ⚠ 临时停用状态（2026-09-23，用户要求实测误报情况后恢复）
   ============================================================
   停用的做法是把它换成一个**永不匹配**的正则，
   下面 4 处引用（tvClean 里 2 处 + 1341 / 1492）原样保留、照常执行，
   只是永远命中不了，等价于没有过滤。

   ⚠⚠ **绝对不要改成「把这一行注释掉」。**
      注释掉声明之后，下面那 4 处 TV_ADULT.test(...) 就变成了引用
      未声明的标识符 —— ES module 必然是严格模式，这会抛 ReferenceError，
      **整个 Worker 直接 500，音乐和影视一起挂**（2026-09-23 已经这么挂过一次）。

   恢复过滤：把下面这行换回原来的正则即可：
       const TV_ADULT = /伦理|福利|里番|情色|成人|无码|色情|自拍|偷拍|人妖|淫/
   ============================================================ */
const TV_ADULT = /(?!)/   /* 永不匹配 —— 过滤处于停用状态 */
