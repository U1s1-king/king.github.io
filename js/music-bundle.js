...
/* 注意 [^\S\r\n]* 而不是 \s*：\s 会把行尾换行也吃掉，于是「只有时间戳、后面没内容」的
     那一行会连下一行的歌词一起吞进来，界面上就会看到歌词前挂着 [00:12.06]。 */
  var re = /\[(\d{1,2}):(\d{1,2})(?:[.:](\d{1,3}))?\][^\S\r\n]*(.*)/g;
var m;
/* 开头几秒的「作词/作曲/编曲」是元信息不是歌词，显示出来会让人以为歌词没对上；
   只在前 20 秒内过滤，避免误伤真正的歌词行。 */
var credit = /^(作词|作詞|作曲|编曲|編曲|制作人|製作人|制作|製作|混音|母带|母帶|录音|錄音|监制|監製|出品|发行|發行|统筹|統籌|企划|企劃|策划|策劃|原唱|演唱|配唱|合声|合聲|吉他|贝斯|貝斯|鼓|键盘|鍵盤|和声|和聲|弦乐|弦樂|OP|SP|词|詞|曲)[^:：]{0,4}[:：]/;
